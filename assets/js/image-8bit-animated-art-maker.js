  ...
  if (file) {
    ...
    reader.onload = function (e) {
      ...
      img.onload = function () {
        ...
        imgCanvas = eightBit(canvas, img, 24)
        for (let index = 1; index < 25; index++) {
          let canvas = document.createElement('canvas')
          let pixelSize = index
          let returnedValue = eightBit(canvas, img, pixelSize)
          gifCanvas.push(returnedValue)
        }
        gifshot.createGIF(
          {
            images: [...gifCanvas],
  ...
  if (fileType === 'gif') {
    ...
    fetch(url)
      ...
      .then((bloburl) => {
        ...
        a.download = `safeimagekit-8bit-animated-art.gif`
    ...
    let a = document.createElement('a')
    a.href = url
    a.download = `safeimagekit-8-bit-art-${fileType}`
    document.body.appendChild(a)
    a.click()
    if (lang === 'en') {
      window.location.href = `/download?tool=${pageTool}`
